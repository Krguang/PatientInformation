...
static uint8_t GetGBKCode_SD(unsigned char* pBuffer,const uint8_t *pstr,uint8_t font)
{
  ...
  if(font==16)
  {
    ...
    f_res=f_open(&SDFile,"16.FON",FA_OPEN_EXISTING|FA_READ);
  }
  else if(font == 24)
  {
    // 24*24大小的汉字 其字模 占用24*24/8个字节
    pos=((high8bit-0xa1)*94+low8bit-0xa1)*24*24/8; 
    f_res=f_open(&SDFile,"24.FON",FA_OPEN_EXISTING|FA_READ);
		
  }else if(font == 32){
		pos=((high8bit-0xa1)*94+low8bit-0xa1)*32*32/8; 
    f_res=f_open(&SDFile,"32.FON",FA_OPEN_EXISTING|FA_READ);
	} else {
		pos=((high8bit-0xa1)*94+low8bit-0xa1)*64*64/8; 
    f_res=f_open(&SDFile,"64.FON",FA_OPEN_EXISTING|FA_READ);
  ...
  if(f_res == FR_OK ) 
  {
    ...
    if(font==16)
    {
      f_res=f_read(&SDFile,pBuffer,32,&f_num); 
    }
    else if(font ==24)
    {
      f_res=f_read(&SDFile,pBuffer,72,&f_num);
    }
		else if (font ==32){
			f_res=f_read(&SDFile,pBuffer,128,&f_num);
		}
		else{
			f_res=f_read(&SDFile,pBuffer,512,&f_num);
		}
